<div class="manager_content accounts-browser">
  <div class="accounts-page-header">
    <div class="accounts-page-title">
      <span class="accounts-page-icon accounts-page-icon-muted"><i class="ti ti-user-off"></i></span>
      <div>
        <h2>{echo phrase="INACTIVE_ACCOUNTS"}</h2>
        <p>Search and review inactive customer accounts.</p>
      </div>
    </div>
  ...
  <div class="accounts-list">
    <div class="accounts-list-header">
      <h3>Inactive account list</h3>
      <p>Customer records that are no longer active.</p>
    </div>
    <div class="table-responsive accounts-table-wrap">
      {form name="inactive_accounts"}
        {form_table field="accounts" size="10" class="table table-hover align-middle mb-0" empty="No inactive accounts found."}
          ...
          {form_table_column columnid="billingstatus" header="[BILL]"}
            <span class="account-status account-status-default"><i></i>{$accounts.billingstatus}</span>
          {/form_table_column}
