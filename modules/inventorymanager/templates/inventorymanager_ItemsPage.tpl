  ...
  <div class="subscriptionmanager-shell">
    ...
    <div class="subscriptionmanager-grid subscriptionmanager-grid-narrow">
      ...
      <section class="subscriptionmanager-card subscriptionmanager-card-table">
        ...
        <div class="subscriptionmanager-table-wrap"><table class="table subscriptionmanager-table"><thead><tr><th>Product</th><th>Item</th><th>Location</th><th>Qty</th></tr></thead><tbody>
          {foreach from=$productMaps item=map}<tr><td>#{$map.productid} {$map.product_name}</td><td>#{$map.itemid} {$map.sku} {$map.item_name}</td><td>{if $map.location_name}{$map.location_name}{else}Default{/if}</td><td>{$map.quantity}</td></tr>{/foreach}
        </tbody></table></div>
      </section>
    </div>
  </div>
